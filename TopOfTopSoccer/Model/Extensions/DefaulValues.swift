...
class UserDefaultManager
{
    ...
    class func getCustomObjFromUserDefaults(key: String) -> AnyObject {
        let defaults = UserDefaults.standard
        if let decoded  = defaults.object(forKey: key) as? NSData{
    ...
    class func setCustomArrayToUserDefaults(array: NSMutableArray , key: String) {
        let defaults = UserDefaults.standard
    ...
    class func getCustomArrayFromUserDefaults(key: String) -> NSMutableArray {
        let defaults = UserDefaults.standard
        if let decoded  = defaults.object(forKey: key) as? NSData{
            let decodedTeams = NSKeyedUnarchiver.unarchiveObject(with: decoded as Data)! as! NSMutableArray
            return decodedTeams
        }
        return []
    }
